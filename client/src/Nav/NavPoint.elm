...
type Elev = ElevMeters Float | ElevFeet Float
type RwLen = LenMeters Float | LenNauticalMiles Float | LenStatuteMiles Float
type alias NavPoint = 
  { name: String
  , code: String
...
navPointToGeoPoint : NavPoint -> GeoPoint
navPointToGeoPoint np = 
  { lon = np.lon
  , lat = np.lat 
  }
...
nameParser = 
  let
    validChar : Char -> Bool
    validChar c = Char.isAlphaNum c || (Set.fromList ['\'', '_', '-'] |> Set.member c)
  in
    variable 
      { start = validChar 
      , inner = validChar
