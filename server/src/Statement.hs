...
combineFlightTaskRows rows =
    ...
    let
        combine :: [Entity Int32 FlightTask] -> Entity Int32 FlightTask -> [Entity Int32 FlightTask]
        combine all currentEntity@(Entity currentKey (FlightTask _ currentTurnpoints _)) =
            case all of
                ...
                (Entity lastKey (FlightTask start turnpoints finish)) : rest | lastKey == currentKey ->
...
saveFlightTaskTurnpointsStatement =
    lmap
        (\(Entity key (FlightTask _ turnpoints _)) ->
            let
                tpsv = (V.indexed . V.fromList) turnpoints
            in
            ( key <$ tpsv
            , name . fst . snd <$> tpsv
